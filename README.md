...
**기존 사용자 자동 이관:** 예전 버전(v8 이하)은 패키지 안 `notes/`에 저장했습니다.
업데이트 후 첫 실행 때 자동으로 새 위치로 옮기고 콘솔에 로그를 남깁니다. 새
위치에 이미 데이터가 있으면 덮어쓰지 않고 양쪽 경로만 안내하니, 그때는 직접
병합하세요.
...
```
ComfyUI/user/bmk_notes/
├── .bmk_meta.json            카테고리/탭 순서, 접힘 상태
├── .bmk_param_presets.json   워크플로우 파라미터 프리셋
├── .trash/                   삭제된 노트 보관 (안전망)
└── <카테고리>/.../<탭>.json   실제 문서 (정본)
    <카테고리>/.../<탭>.txt    사람용 읽기 사본 (json→txt 단방향 미러)
```

`.txt`는 ComfyUI를 켜지 않고 프롬프트를 에디터로 복사하기 위한 것입니다. json
저장 시마다 자동 갱신되며, txt를 직접 고쳐도 json에는 반영되지 않습니다.
