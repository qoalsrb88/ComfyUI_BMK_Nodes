...
v4 (2026-09)
------------
- 빈 마스크 무시: mask 입력이 연결되어 있어도 값이 전부 0(영역 미지정)이면
  마스크 없음으로 취급해 크롭 전체를 붙인다. 이전에는 로더의 mask 출력
  (알파 없는 이미지 → 0 마스크)을 연결해 두면 결과가 원본 그대로 나와서
  포트를 끊어야만 크롭이 반영되는 문제가 있었다. 무시 시 로그를 남긴다.
...
class BMKCropStitch:
    ...
    def stitch(
        ...
        # 크롭이 반영되지 않는 것처럼 보인다. 포트를 연결만 해 둔 상태에서도
        # 크롭 전체가 붙도록 여기서 해제한다. (영역이 지정된 마스크는 그대로)
        if mask is not None and not bool((mask > 0).any()):
            logger.info(
                "%s mask 입력이 전부 0(영역 미지정)이라 무시하고 크롭 전체를 "
                "붙입니다. 일부만 붙이려면 마스크에 영역(1)을 지정하세요.",
                _TAG,
            )
            mask = None
        if mask is not None:
